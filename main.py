...
def _build_svg_path(text, cx, cy, font_size, anchor, fill):
    ...
    d = []; i = 0; n = len(codes)
    while i < n:
        code = codes[i]; vx, vy = verts[i]
        sx = vx + ox;    sy = oy - vy
        if code == MplPath.MOVETO:
            d.append(f'M {sx:.2f},{sy:.2f}'); i += 1
        elif code == MplPath.LINETO:
            d.append(f'L {sx:.2f},{sy:.2f}'); i += 1
        elif code == MplPath.CURVE3:
            vx2, vy2 = verts[i+1]
            d.append(f'Q {sx:.2f},{sy:.2f} {vx2+ox:.2f},{oy-vy2:.2f}'); i += 2
        elif code == MplPath.CURVE4:
            vx2, vy2 = verts[i+1]; vx3, vy3 = verts[i+2]
            d.append(f'C {sx:.2f},{sy:.2f} {vx2+ox:.2f},{oy-vy2:.2f} {vx3+ox:.2f},{oy-vy3:.2f}'); i += 3
...
def add_text_dxf(msp, text, cx, cy, font_size, color=7):
    ...
    verts = mtp.vertices; codes = mtp.codes
    i = 0; n = len(codes); current = []
    ...
    while i < n:
        ...
        px = vx + ox; py = vy + oy
        if code == MplPath.MOVETO:
            flush(current); current = [(px, py)]; i += 1
        elif code == MplPath.LINETO:
            current.append((px, py)); i += 1
        elif code == MplPath.CURVE3:
            p0 = current[-1] if current else (px, py)
            vx2, vy2 = verts[i+1]; p1=(px,py); p2=(vx2+ox, vy2+oy)
            ...
            p0 = current[-1] if current else (px, py)
            vx2,vy2=verts[i+1]; vx3,vy3=verts[i+2]
            p1=(px,py); p2=(vx2+ox,vy2+oy); p3=(vx3+ox,vy3+oy)
            ...
            if current: current.append(current[0])
            flush(current); current = []; i += 1
        else:
            i += 1
    flush(current)
...
def build_elements(zones, title, tool_number, date_str,
    ...
    CT  = int(svg_h * 0.396)   # chevron top
    CB  = int(svg_h * 0.672)   # chevron bottom
    JY  = (CT + CB) // 2
...
def render_svg(elements, svg_w, svg_h, stroke, bg, inactive_col,
    ...
    for e in elements:
        ...
        if t == 'line':
            ...
            L.append(f'<line x1="{e["x1"]}" y1="{e["y1"]}" x2="{e["x2"]}" y2="{e["y2"]}" '
                     f'stroke="{col}" stroke-width="{lw}"/>')
        elif t == 'circle':
            fc = bg
            L.append(f'<circle cx="{e["cx"]}" cy="{e["cy"]}" r="{e["r"]}" '
                     f'fill="{fc}" stroke="{stroke}" stroke-width="1.8"/>')
        elif t == 'rect':
            L.append(f'<rect x="{e["x"]}" y="{e["y"]}" width="{e["w"]}" height="{e["h"]}" '
...
def render_dxf(elements, svg_w, svg_h, stroke,
    ...
    def ds(v): return v * dxf_scale
    def tx_(x): return ds(x) + offset_x
    def ty_(y): return ds(fy(y)) + offset_y

    # Optional plate border on layer PLATE_BORDER
    if draw_border and plate_w_mm and plate_h_mm:
        doc.layers.add('PLATE_BORDER', color=3, lineweight=25)
        msp.add_lwpolyline([
            (0, 0), (plate_w_mm, 0),
            (plate_w_mm, plate_h_mm), (0, plate_h_mm), (0, 0)
...
tpl_cfg = {
    "title": title_text, "tool_number": tool_number,
    "num_zones": num_zones, "zone_width": zone_width, "svg_height": svg_height,
    "show_polarity": show_polarity, "show_zone_lbl": show_zone_lbl,
    "heater_dividers": heater_dividers,
    "stroke_color": stroke_color, "bg_color": bg_color,
    "inactive_color": inactive_color, "font_size_num": font_size_num,
